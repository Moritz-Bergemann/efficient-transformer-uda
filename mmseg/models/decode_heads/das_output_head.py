...
class DASOutputHead(DASHead):
    """
    Basic Domain Adversarial SegFormer head.
    """

    def __init__(self, **kwargs):
        super(DASOutputHead, self).__init__(**kwargs)
        ...
        discriminator_params['in_channels'] = self.num_classes
    ...
    def forward(self, inputs, compute_seg=True, compute_disc=True):
        ...
        # In this version, we must always compute segmentation, since segmentation is
        # passed into the discriminator. Therefore, no "if compute_seg:"
        ...
        if compute_disc:
            ...
            domain_pred = self.discriminator(seg_pred) # Apply discriminator to the softmax output predictions
        else:
            domain_pred = None
