...
class MultiHeadCrossentropyWrapper(nn.Module):
    def __init__(self, **cfg):
        self.crossentropy = build_loss(cfg['crossentropy'])
    ...
    def forward(self,                 
        ...
        """Compute crossentropy dynamically across the multiple provided losses."""
        
        label = MultiHeadCrossentropyWrapper.expand_dim(label, cls_score.shape[-1])
        ...
        for i in range(head_count):
            loss += self.crossentropy(cls_score[i], label, weight, avg_factor, reduction_override)
        ...
        self.crossentropy(cls_score, label, weight, avg_factor, reduction_override)
